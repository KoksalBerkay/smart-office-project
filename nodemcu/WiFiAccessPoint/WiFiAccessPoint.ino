  ...
  if (client) {                             // if you get a client,
    ...
    while (client.connected()) {            // loop while the client's connected
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (currentLine.endsWith("GET /RONN")) {
          client.println("XONX");
        }
        if (currentLine.endsWith("GET /ROFF")) {
          client.println("XOFFX");
        }
        if(currentLine.endsWith("GET /CHANGE")){
          //char data[10] = {10,20,30,0,0,0,0,0,0,0};
          int temperature = 30;
          //client.println("<p>Temperature: 31 C</p>");
          client.println("<p>" + String(temperature) + "</p>" );
